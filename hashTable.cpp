...
HashTable::HashTable() { //maybe we need a try and catch here
    size = INITIAL_SIZE;
    table = new OGAVLTree<int, std::shared_ptr<Customer>>[size]();
    elementCount = 0;
}
...
void HashTable::resize(int updatedSize) {
    int oldSize = size;
    size = updatedSize;
    OGAVLTree<int, std::shared_ptr<Customer>>* newTable = new OGAVLTree<int, std::shared_ptr<Customer>>[updatedSize];
